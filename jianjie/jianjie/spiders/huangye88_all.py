...
from jianjie.items import Huangye88AllItem
...
class TouzishijianSpider(scrapy.Spider):
	...
	def start_requests(self):
		start_url = "http://b2b.huangye88.com/region/"
		yield scrapy.Request(start_url)

	def parse(self, response):
		sel = Selector(text=response.text)
		a_tags = sel.xpath('//dl[@id="clist"]/dd/a')
		for a in a_tags:
			item = Huangye88AllItem()
	...
	def parse_list(self, response):
		...
		yield scrapy.Request(self.burl.format(cat, (pn_now + 1)), callback=self.parse_list, meta={'cat': cat, 'item': item})
